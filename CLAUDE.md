...
### Routing & project posts (MDX)

`App.tsx` is now a `BrowserRouter` with two routes: `/` → `pages/HomePage.tsx` (the full solar-system experience) and `/projects/:slug` → `pages/ProjectPostPage.tsx` (an individual post). The scroll/collision providers wrap **`HomePage`, not the app root**, so post pages stay lightweight.

- **Posts** live in `src/content/projects/*.mdx`. The filename is the slug. YAML frontmatter (`title`, `date`, `summary`, `tags?`, `cover?`, `draft?`) is parsed by `remark-mdx-frontmatter` and re-exported; `src/mdx.d.ts` declares the `*.mdx` module shape. `content/projects/index.ts` builds the typed `projects` registry via `import.meta.glob('./*.mdx', { eager: true })` (newest first, drafts filtered) — bodies ride in the main bundle; if the blog grows large, switch to a metadata-only registry + `() => import(...)` loaders to code-split.
- **MDX pipeline** is configured in `vite.config.ts`: `@mdx-js/rollup` (enforce `pre`) with `remark-gfm`, frontmatter plugins, `rehype-slug`, and `rehype-highlight`. Element overrides (lazy images, safe external links) are in `components/mdx/MdxComponents.tsx`, applied via `<MDXProvider>`. Post body styling uses the `@tailwindcss/typography` `prose prose-invert` classes; the highlight.js theme is imported in `main.tsx`.
- **GitHub Pages SPA fallback:** clean URLs need deep-link support on a static host. `public/404.html` encodes the path into a query and redirects to `/`; a snippet in `index.html` decodes it back before the app boots (rafgraph/spa-github-pages; root domain → `pathSegmentsToKeep = 0`).

### Context providers (wrap `HomePage` in `pages/HomePage.tsx`)
...
- **Add a project post:** drop a `src/content/projects/<slug>.mdx` file with frontmatter (`title`, `date`, `summary`, `tags`); the card and the `/projects/<slug>` page appear automatically (newest first). Set `draft: true` to hide it. It's MDX, so you can embed JSX/components and `<video>` directly.
- **Add a publication:** append a `Publication` (type in `types/index.ts`) to `src/data/publications.ts`; the Publications section renders a citation card (authors with `SELF_AUTHOR` emphasized, collapsible abstract, IEEE/DOI links). Unlike projects, publications are a data-driven list that links out — no per-publication page.
